...
from .models import User, Product, Brand, Gender
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ('id', 'category', 'brand', 'gender', 'name', 'slug', 'price', 'available', 'stock', 'created_at')
...
class BrandSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        return Brand(**validated_data)

def slug_must_be_lower(value):
    for i in value:
        if i.isupper():
            raise serializers.ValidationError('slug must be lower')

class GenderSerializer(serializers.Serializer):
    name = serializers.CharField(max_length=100)
    slug = serializers.SlugField(max_length=200, validators=[slug_must_be_lower])
    ...
    def validate_name(self, value):
        ...
        """
        if 'male' not in value.lower():
            raise serializers.ValidationError("Gender name must contain the word 'male' ")
        return value

    def create(self, validated_data):
        # Валидация на уровне объекта
        if len(validated_data['slug']) > len(validated_data['name']):
            raise serializers.ValidationError("slug more than name")
        return Gender(**validated_data)
